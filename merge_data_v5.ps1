$jsonData = Get-Content -Path "extracted_data.json" -Raw | ConvertFrom-Json
...
$finalJs = $header + "`n`nconst SAMPLE_DEMANDAS = " + $demandasJson + ";`n`nconst SAMPLE_NOTEBOOKS = " + $notebooksJson + ";`n`n" + $footer

[System.IO.File]::WriteAllText("avb-gestao\data.js", $finalJs)
